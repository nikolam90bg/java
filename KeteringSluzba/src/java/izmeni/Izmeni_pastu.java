 ...
 * @author Nikola90
 */
public class Izmeni_pastu extends HttpServlet {
        ...
        if (naziv != null && naziv.length() > 0 && slika != null && slika.length() > 0 && opis != null && opis.length() > 0 && cenas != null && cenas.length() > 0) {
            try {
                ...
                try {
                    ...
                    String upit = "update pasta set naziv=?,slika=?, opis=?, cena=? where sifra='" + sifra + "'";
                    ps = con.prepareStatement(upit);

                    ps.setString(1, naziv);
                    ps.setString(2, "images/pasta/" + slika);
                    ps.setString(3, opis);
                    ps.setFloat(4, cena);
                ...
                RequestDispatcher rd = request.getRequestDispatcher("izmeniPastu.jsp");
