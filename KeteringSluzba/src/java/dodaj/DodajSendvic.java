 ...
 * @author Nikola90
 */
public class DodajSendvic extends HttpServlet {
        ...
        if (sifra != null && sifra.length() > 0 && naziv != null && naziv.length() > 0 && slika != null && slika.length() > 0 && opis != null && opis.length() > 0 && cenas != null && cenas.length() > 0) {
            ...
            try {
                ...
                try {
                    ...
                    String upit = "insert into sendvic(sifra, naziv, slika, opis, cena)"
                            + "values(?,?,?,?,?)";
                    ps = con.prepareStatement(upit);

                    ps.setString(1, sifra);
                    ps.setString(2, naziv);
                    ps.setString(3, "images/sendvic/" + slika);
                    ps.setString(4, opis);
                    ps.setFloat(5, cena);
                    ...
                    RequestDispatcher rd = request.getRequestDispatcher("dodajSendvic.jsp");
